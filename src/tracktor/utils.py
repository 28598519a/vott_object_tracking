...
def plot_sequence_video(tracks, db, output_dir):
    """Plots a whole sequence
    ...
    print("[*] Plotting whole sequence (video)to {}".format(output_dir))

    frame_threshold_to_split = 10

    if not osp.exists(output_dir):
        os.makedirs(output_dir)

    dets = {} # keys is track id, then inside is two keys 'ims', 'last_frame'
    ...
    for i, v in enumerate(db):
        ...
        for j, t in tracks.items():
            if i in t.keys():
                ...
                #first time
                if not j in dets.keys():
                    dets[j] = {}
                    x1 = int(np.clip(t_i[0], 0, im.shape[0]))
                    x2 = int(np.clip(t_i[0]+t_i[2], 0, im.shape[0]))
                    y1 = int(np.clip(t_i[1], 0, im.shape[1]))
                    y2 = int(np.clip(t_i[1]+t_i[3], 0, im.shape[1]))

                    dets[j]['ims'] = [im[x1:x2, y1:y2]]
                    dets[j]['first_frame'] = int(i)
                    dets[j]['last_frame'] = int(i)
                
                #not first time
                elif j in dets.keys():
                    if (i - dets[j]['last_frame']) > frame_threshold_to_split:
                        #split
                        #seq_name + track_id + first_frame + last_frame 
                        output_name = str(db._seq_name) + '_' + str(j) + '_' + str(dets[j]['first_frame']) + '_' + str(dets[j]['last_frame']) + '.mp4'
                        write_ims_to_video(dets[j]['ims'], output_dir=output_dir, output_name=output_name)
                    else:
                        #still in 10 frame gap, connected together
                        dets[j]['ims'].append(im[x1:x2, y1:y2]) 
                        dets[j]['last_frame'] = int(i)

def write_ims_to_video(ims, output_dir, output_name):
    fourcc = cv2.VideoWriter_fourcc('m', 'p', '4', 'v')
    fps = 15
    
    ims = np.array(ims)
    avr_size = np.average([im.shape[:2] for im in ims], axis=0)
    avr_size = np.rint(avr_size).astype(int)
    print('video name: ', osp.join(output_dir, output_name))
    out = cv2.VideoWriter(osp.join(output_dir, output_name), fourcc, fps, avr_size)
    for im in ims:
        im = cv2.resize(im, avr_size)
        out.write(im)
