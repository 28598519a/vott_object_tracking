...
class Tracker:
	...
	def step(self, blob, bb):
		...
		# det_pos use two-level list; det_scores use one-level list
		# bb <left><top><left + width><top + height>
		det_pos = torch.tensor(bb, device = torch.device("cuda"))
		det_scores = torch.tensor(scorelist, device = torch.device("cuda"))
		...
		if len(self.tracks):
			...
			if self.do_align:
				try:
					...
					# error: (-7:Iterations do not converge)
					pass
		...
		for t in self.tracks:
			...
			# Record the id of the frame
			search[int(self.results[t.id][self.im_index][0])] = int(t.id)
		
		# GIGO
		for i in range(len(bb)):
			id.append(search[int(bb[i][0])])
